...
namespace CSC.Core
{
    public class UserInfoManager : BaseManager<UserInfo>
    {
        ...
        /// <returns></returns>
        public Paging<UserInfo> FindPageList(Paging<UserInfo> pagingUser, Guid? roleID, string username, string name, int? sex, string email, int? order)
        {
            //查询表达式
            var _where = PredicateBuilder.True<UserInfo>();
            //if (roleID != null && roleID > 0) _where = _where.And(u => u.RoleId == roleID);
            if (roleID != null) _where = _where.And(u => u.RoleId == roleID);
        ...
        /// <returns></returns>
        public override Response Add(UserInfo user)
        {
