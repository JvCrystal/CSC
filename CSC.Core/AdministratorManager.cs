...
namespace CSC.Core
{
    public class AdministratorManager : BaseManager<Administrator>
    {
        ...
        /// <returns></returns>
        public Response ChangePassword(Guid administratorID, string password)
        {
            Response _resp = new Response();
        ...
        /// <param name="administratorID">主键</param>
        /// <returns></returns>
        public override Response Delete(Guid administratorID)
        {
            Response _resp = new Response();
            if (Count() == 1)
            {
        ...
        /// <returns></returns>
        public Response Delete(List<Guid> administratorIDList)
        {
            Response _resp = new Response();
            int _totalDel = administratorIDList.Count;
            int _totalAdmin = Count();
            foreach (Guid i in administratorIDList)
            {
                if (_totalAdmin > 1)
                {
        ...
        /// <returns></returns>
        public Response UpadateLoginInfo(Guid administratorID, string ip, DateTime time)
        {
            Response _resp = new Response();
            var _admin = Find(administratorID);
            if (_admin == null)
